...
func NewEOSBlockMapper(hooksActionName string, restrictions []*Restriction) *EOSBlockMapper {
	return &EOSBlockMapper{hooksActionName: hooksActionName, restrictions: restrictions}
}

func (m *EOSBlockMapper) IndexMapping() *mapping.IndexMappingImpl {
	// db ops
	dbDocMapping := bleve.NewDocumentMapping()
	dbDocMapping.AddFieldMappingsAt("key", txtFieldMapping)
	dbDocMapping.AddFieldMappingsAt("table", txtFieldMapping)

	// ram ops
	ramDocMapping := bleve.NewDocumentMapping()
	ramDocMapping.AddFieldMappingsAt("consumed", txtFieldMapping)
	ramDocMapping.AddFieldMappingsAt("released", txtFieldMapping)

	// Root doc
	rootDocMapping := bleve.NewDocumentStaticMapping()

	// Sortable internals
	rootDocMapping.AddFieldMappingsAt("act_idx", sortableNumericFieldMapping)
	rootDocMapping.AddFieldMappingsAt("block_num", sortableNumericFieldMapping)
	rootDocMapping.AddFieldMappingsAt("trx_idx", sortableNumericFieldMapping)

	rootDocMapping.AddFieldMappingsAt("receiver", txtFieldMapping)
	rootDocMapping.AddFieldMappingsAt("account", txtFieldMapping)
	rootDocMapping.AddFieldMappingsAt("action", txtFieldMapping)
	rootDocMapping.AddFieldMappingsAt("auth", txtFieldMapping)
	rootDocMapping.AddFieldMappingsAt("input", boolFieldMapping)
	rootDocMapping.AddFieldMappingsAt("notif", boolFieldMapping)
	rootDocMapping.AddFieldMappingsAt("scheduled", boolFieldMapping)

	// add other sub-sections here
	rootDocMapping.AddSubDocumentMapping("data", dynamicNestedDocMapping)
	rootDocMapping.AddSubDocumentMapping("db", dbDocMapping)
	rootDocMapping.AddSubDocumentMapping("ram", ramDocMapping)
	rootDocMapping.AddSubDocumentMapping("event", dynamicNestedDocMapping)

	// this disables the _all field
	rootDocMapping.AddSubDocumentMapping("_all", disabledMapping)
