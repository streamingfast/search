...
func TestRunQuery(t *testing.T) {
	...
	tests := []struct {
		poolSetup             func(t *testing.T, pool *IndexPool)
		queries               []query
...
`)
	...
	for idx, test := range tests {
		t.Run(fmt.Sprintf("index %d", idx+1), func(t *testing.T) {
			...
			client, cleanup := newTestClient(t, &ArchiveBackend{pool: pool, maxQueryThreads: 2, searchPeer: pool.SearchPeer}, pbbstream.Protocol_EOS)
			defer cleanup()
...
func appendTestIndex(t *testing.T, tmpDir string, typ string, pool *IndexPool, startBlock, endBlock uint64, indexName string, content string) *search.ShardIndex {
	t.Helper()
	...
	mapper := search.NewEOSBlockMapper("", nil).IndexMapping()
